...
$idFacturacion_folio = $idFacturacion + 1;
...
foreach ($operacion as $key) {
  ...
  $saveOperation = mysql_query("INSERT INTO operacion_cliente (numeroOperacion, idCliente, idFacturacion) 
                                VALUES ('$key', '$idclientes','0')");
...
//Begin foreach_3
foreach ($operacion as $keyPathFiles) {
  $updateXMLPDF = mysql_query("UPDATE operacion_cliente 
                               SET xml     =  '$namexml',
                               pdf         =  '$pathPDF',
                               rfc_factura =  '$rfc_factura' 
                               WHERE idCliente = '$idclientes' 
                               AND numeroOperacion = '$keyPathFiles'");

  $url = "http://gnuvehicular.mine.nu:8580/ventas_general.php?operacion=$keyPathFiles";
  include('call-cross-domain.php');
  ...
  $mail->Subject = iconv('UTF-8', 'ISO-8859-1', "Datos de facturación | Folio ".$idFacturacion_folio);
